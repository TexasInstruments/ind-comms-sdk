...
- To run this demo, Linux needs to run on the Cortex A-core. Assuming the boot media is SD card, refer to **Processor SDK Linux** user guide on how create SD cards to boot up linux.
- Copy the generated .out file into the Linux filesystem under /lib/firmware in the boot media "root" partition.
- Boot the remote core (r5fss0_0) from Linux by following the steps from <a href="https://dev.ti.com/tirex/explore/node?a=7qm9DIS__LATEST&node=A__AdAyuKWUWVV5j4wBc7C6XA__AM64-ACADEMY__WI1KRXP__LATEST" target="_blank"> AM64x Academy </a>
...
Once Linux is loaded onto the A53 cores of AM64x and the RemoteProc driver boots the remote core with our application binaries during Linux kernel boot time. The following logs appear on UART0.
