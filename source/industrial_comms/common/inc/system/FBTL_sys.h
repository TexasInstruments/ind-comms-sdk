...
/*!
    ...
    \ingroup FBTL_SYS
*/
typedef enum FBTL_SYSLIB_Irq
{
    FBTL_SYSLIB_IRQ_cpu2app = 0x1000,   ///!<   IRQ provider to remote
    FBTL_SYSLIB_IRQ_app2cpu = 0x2000,   ///!<   IRQ remote to provider
} FBTL_SYSLIB_Irq_t;
...
/* Underlay */
typedef uint64_t (*FBTL_SYS_underLaySize_t)(void *pContext);
typedef volatile uint8_t *(*FBTL_SYS_getUnderLay_t)(
    void *pContext,
    uint64_t offset,
    uint64_t size,
    bool* pIsDirectAccess);
typedef uint32_t (*FBTL_SYS_setUnderLay_t)(
    void *pContext,
...
/* Memory manager */
typedef uint64_t (*FBTL_SYS_getSectionBase_t)(
    void *pContext,
    FBTL_SYSLIB_eMemorySection_t memSection);
typedef uint64_t (*FBTL_SYS_getSectionSize_t)(
    void *pContext,
    FBTL_SYSLIB_eMemorySection_t memSection);

/* Memory */
typedef uint8_t *(*FBTL_SYS_malloc_t)(void *pContext, uint32_t size);
typedef void (*FBTL_SYS_free_t)(void *pContext, uint8_t *pPtr);
typedef void (*FBTL_SYS_memset_t)(void *pContext, uint8_t *pPtr, uint8_t value, uint32_t num);
typedef void (*FBTL_SYS_memcpy_t)(
    void *pContext,
...
/* Interrupts */
typedef void *(*FBTL_SYS_IRQ_create_t)(
    void *pContext,
    bool outNotIn,
    FBTL_SYSLIB_Irq_t irqType);
typedef void (*FBTL_SYS_IRQ_cbHookIsr_t)(void *pContext);
typedef void (*FBTL_SYS_IRQ_registerISR_t)(
    void *pIrq,
    void *pIsrContext,
    FBTL_SYS_IRQ_cbHookIsr_t cbIsrCallback);
typedef void (*FBTL_SYS_IRQ_delete_t)(void *pContext, void *pIrq);
typedef uint32_t (*FBTL_SYS_IRQ_wait_t)(void *pContext, void *pIrq, uint32_t timeout);
typedef void (*FBTL_SYS_IRQ_post_t)(void *pContext, void *pIrq);
typedef void (*FBTL_SYS_IRQ_handshake_t)(void *pContext);
typedef bool (*FBTL_SYS_IRQ_inISR_t)(void *pContext);

/* Signals */
typedef void *(*FBTL_SYS_SIG_create_t)(void *pContext, uint8_t spawn, char *pName);
typedef void (*FBTL_SYS_SIG_delete_t)(void *pContext, void *pSignal);
typedef uint32_t (*FBTL_SYS_SIG_wait_t)(void *pContext, void *pSignal, uint32_t timeout);
typedef void (*FBTL_SYS_SIG_post_t)(void *pContext, void *pSignal);
typedef void *(*FBTL_SYS_CSIG_create_t)(void *pContext, uint32_t countStart, uint32_t countMax);

/* Mutex */
typedef void *(*FBTL_SYS_MTX_create_t)(void *pContext, uint8_t spawn, char *pName);
typedef void (*FBTL_SYS_MTX_delete_t)(void *pContext, void *pSignal);
typedef uint32_t (*FBTL_SYS_MTX_lock_t)(void *pContext, void *pSignal, uint32_t timeout);
typedef void (*FBTL_SYS_MTX_unLock_t)(void *pContext, void *pSignal);

typedef void *(*FBTL_SYS_CS_create_t)(void *pContext);
typedef void (*FBTL_SYS_CS_delete_t)(void *pContext, void *pCritSect);
typedef uint32_t (*FBTL_SYS_CS_enter_t)(void *pContext, void *pCritSect, uint32_t timeout);
typedef void (*FBTL_SYS_CS_leave_t)(void *pContext, void *pCritSect);

/* Scheduler */
typedef void (*FBTL_SYS_SCHED_yield_t)(void *pContext);
typedef void (*FBTL_SYS_SCHED_sleep_t)(void *pContext, uint32_t msTimeout);
typedef uint32_t (*FBTL_SYS_SCHED_getThreadId_t)(void *pContext);
typedef bool (*FBTL_SYS_SCHED_isIdentThrdId_t)(void *pContext, uint32_t threadId);
typedef uint32_t (*FBTL_SYS_getSysTickMs_t)(void *pContext);
typedef uint32_t (*FBTL_SYS_setCpuSpeed_t)(void *pContext, uint32_t frequency);
typedef uint32_t (*FBTL_SYS_getCpuSpeed_t)(void *pContext, uint32_t *pFrequency);

typedef void (*FBTL_SYS_printf_t)(void *pContext, const char *pFormat, va_list vaarg);
...
typedef struct FBTL_SYSLIB_SRamUnderlay
{
    ...
    uint64_t ramSize;                   ///!<   size of shared RAM
...
extern void FBTL_SYS_printf(void *pSysLibHandle, const char *pFormat, ...);
...
extern volatile uint8_t *FBTL_SYS_getUnderLay(
    void *pSysLibHandle,
    uint64_t offset,
    uint64_t size,
    bool *pIsDirectAccess);
extern uint32_t FBTL_SYS_setUnderLay(
    void *pSysLibHandle,
    volatile uint8_t *pBuffer,
    uint64_t offset,
    uint64_t size);

extern uint64_t FBTL_SYS_getSectionBase(
    void *pSysLibHandle,
    FBTL_SYSLIB_eMemorySection_t memSection);
extern uint64_t FBTL_SYS_getSectionSize(
    void *pSysLibHandle,
    FBTL_SYSLIB_eMemorySection_t memSection);

/* memory */
extern uint8_t *FBTL_SYS_malloc(void *pSysLibHandle, uint32_t length);
extern void FBTL_SYS_free(void *pSysLibHandle, uint8_t *pData);
extern void FBTL_SYS_memset(void *pSysLibHandle, uint8_t *pPtr, uint8_t value, uint32_t num);
extern void FBTL_SYS_memcpy(
    void *pSysLibHandle,
    uint8_t *pDestination,
    const uint8_t *pSource,
    uint32_t num);
...
/* Interrupts */
extern void *FBTL_SYS_IRQ_create(void *pSysLibHandle, bool outNotIn, FBTL_SYSLIB_Irq_t irqType);
extern void FBTL_SYS_IRQ_delete(void *pSysLibHandle, void *pIrq);
extern uint32_t FBTL_SYS_IRQ_wait(void *pSysLibHandle, void *pIrq, uint32_t timeout);
extern void FBTL_SYS_IRQ_post(void *pSysLibHandle, void *pIrq);
extern void FBTL_SYS_IRQ_handshake(void *pSysLibHandle);
extern bool FBTL_SYS_IRQ_inIsr(void *pSysLibHandle);
extern void FBTL_SYS_IRQ_register(
    void *pSysLibHandle,
    void *pIrq,
    void *pIsrContext,
    FBTL_SYS_IRQ_cbHookIsr_t cbIsr);

/* Signals */
extern void *FBTL_SYS_SIG_create(void *pSysLibHandle, uint8_t spawn, char *pName);
extern void FBTL_SYS_SIG_delete(void *pSysLibHandle, void *pSignal);
extern uint32_t FBTL_SYS_SIG_wait(void *pSysLibHandle, void *pSignal, uint32_t timeout);
extern void FBTL_SYS_SIG_post(void *pSysLibHandle, void *pSignal);

extern void *FBTL_SYS_CSIG_create(void *pSysLibHandle, uint32_t countStart, uint32_t countMax);
extern void FBTL_SYS_CSIG_delete(void *pSysLibHandle, void *pSignal);
extern uint32_t FBTL_SYS_CSIG_wait(void *pSysLibHandle, void *pSignal, uint32_t timeout);
extern void FBTL_SYS_CSIG_post(void *pSysLibHandle, void *pSignal);

/* Mutex */
extern void *FBTL_SYS_MTX_create(void *pSysLibHandle, uint8_t spawn, char *pName);
extern void FBTL_SYS_MTX_delete(void *pSysLibHandle, void *pMutex);
extern uint32_t FBTL_SYS_MTX_lock(void *pSysLibHandle, void *pMutex, uint32_t timeout);
extern void FBTL_SYS_MTX_unLock(void *pSysLibHandle, void *pMutex);

/* Cirtical section */
extern void *FBTL_SYS_CS_create(void *pSysLibHandle);
extern void FBTL_SYS_CS_delete(void *pSysLibHandle, void *pCritSect);
extern uint32_t FBTL_SYS_CS_enter(void *pSysLibHandle, void *pCritSect, uint32_t timeout);
extern void FBTL_SYS_CS_leave(void *pSysLibHandle, void *pCritSect);

/* Scheduler */
extern uint32_t FBTL_SYS_getMsTick(void *pSysLibHandle);

extern uint32_t FBTL_SYS_setCpuSpeed(void *pSysLibHandle, uint32_t frequency);
extern uint32_t FBTL_SYS_getCpuSpeed(void *pSysLibHandle, uint32_t *pFrequency);
extern void FBTL_SYS_yield(void *pSysLibHandle);
extern void FBTL_SYS_sleep(void *pSysLibHandle, uint32_t msTimeout);
extern uint32_t FBTL_SYS_getThreadId(void *pSysLibHandle);
extern bool FBTL_SYS_isIdentThrdId(void *pSysLibHandle, uint32_t threadId);
